...
def send_periodic_emails(config: dict):
    """每10分钟发送一次进度邮件"""
    logger.info("Starting to send periodic emails...")
    while not stop_sending_emails.wait(600):  # 600秒 = 10分钟
        try:
...
def run_graph(config: dict, graph: CompiledStateGraph, save_path: str, init_state: State):
    ...
    try:
        ...
        for event in graph.stream(init_state):
            ...
            if len(list(event.keys())) > 0:
                state_name = list(event.keys())[0]
                frontend_update_node(state_name)
                step_i += 1
