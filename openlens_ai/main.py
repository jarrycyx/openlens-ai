...
def main_resume(
    save_dir: str, refine_suggestion: str = "", start_from_subgraph: str = "", start_from_subtask_index: int = None, interrupt_after_subgraph=""
) -> None:
    ...
    if start_from_subgraph:
        logger.info(f"Resuming from subgraph {start_from_subgraph}")
...
def parse_args():
    ...
    # Execution control
    parser.add_argument("--interrupt-after-subgraph", type=str, default="", help="Interrupt after the specified subgraph, use to limit user trial.")
    parser.add_argument("--start-from-subgraph", type=str, default="", help="Start from the specified subgraph, use to resume from a specific subgraph.")
    parser.add_argument(
